...
class LinearEvaluator(Evaluator):
    ...
    def simple_pre(self, save=True):
        gt = self.gt
        gt_dev = self.dev(gt)
        # reorder in case of boxes
        #gt_dev[:, [3, 1]] = gt_dev[:, [1, 3]]
        #gt_dev[:, 1] = -gt_dev[:, 1]
        es = self.es
        estimated_vel = es[:,-3:]
        estimated_ts = (es[:, 1] + es[:, 2]) / 2
        es = np.c_[estimated_ts, estimated_vel]
        # remove first est values if not gt pendant exists
        while gt_dev[0,0]>es[0,0]:
            es = np.delete(es, (0), axis=0)
        # remove last est values if not gt pendant exists
        while gt_dev[-1, 0] < es[-1, 0]:
            es = np.delete(es, (-1), axis=0)
